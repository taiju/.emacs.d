...
Orgモードで書かれたこの文書は、 ~init.el~ から ~(org-babel-load-file (expand-file-name "~/.emacs.d/config.org"))~ で読み込むこまれることで設定ファイルになる。
...
ansi-term はいつも起動することになるので、最初から起動しておく。

#+BEGIN_SRC emacs-lisp
  (ansi-term "bash")
#+END_SRC

上記のままだと初期画面が ansi-term になってしまって辛いので ~*scratch*~ バッファに切り替える。
...
フルスクリーンだと時間とかバッテリー残量見えなくて困るので、モードラインに表示する。
...
つい癖で半角全角キーで日本語入力を切り替えてしまうので、半角全角キーにも ~toggle-input-method~ を割り当てておく。
...
#+BEGIN_SRC emacs-lisp
  (let ((f "Ricty"))
    ...
    (add-to-list 'face-font-rescale-alist `(,f . 1.0)))
#+END_SRC
...
基本はこれで良いけど、diff 出過ぎて辛いことになる場合は OFF にできるようにカスタムコマンドを定義しておく。
...
コマンド補完等は Ido を全面的に採用する。標準で入っているし、デフォルト厨的に。
...
デフォルトだとサブツリーのバックグラウンドカラーがどぎつくて見にくすぎるので、色変わる必要もないし OFF る。
...
** TODO よく使うプログラミング言語の設定
** TODO まだ出会っていない素敵なパッケージの調査
