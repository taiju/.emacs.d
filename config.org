...
#+begin_src emacs-lisp :noweb yes
  ...
  (defun taiju/go-mode-init ()
    ...
    (setq tab-width 4
          c-basic-offset 4)
    <<taiju/go-mode-init#before-save-hook>>)
  (add-hook 'go-mode-hook #'taiju/go-mode-init)
#+end_src

保存時のフック（go モードローカル）で、フォーマットと import 文の整理が実行されるようにする。

#+begin_src emacs-lisp :noweb-ref taiju/go-mode-init#before-save-hook
