...
#+begin_src emacs-lisp
  (cl-labels ((basename-to-filename (basename)
                                    (concat basename ".org"))
              (filename-to-path (filename)
                                (string-join (list org-directory filename) "/"))
              (path-to-refile-target (path)
                                     (cons path (cons :level 1))))
    (let* ((basenames '("bucket" "wish" "todo" "notes"))
           (filenames (mapcar #'basename-to-filename basenames))
           (paths (mapcar #'filename-to-path filenames))
           (refile-targets (mapcar #'path-to-refile-target paths)))
